...
class Pdu():
    ...
    def applyConfig(self, ask=True):
        ...
        if ask and something_to_do:
            answer = input(colored("DO IT ? [yes|NO] ", "white", attrs=['bold']))
        ...
        if len(outlets) > 0:
            ...
            for o in outlets.keys():
                for oname in outlets[o]:
                    ...
                    msg += ostatus + "\n"
            print(msg)
            answer = input(colored("If you want to ", "white", attrs=['bold']) + colored(action.upper(), "red", attrs=['bold']) + colored(", type ", "white", attrs=['bold']) + colored("YES", "green", attrs=['bold']) + colored(" : ", "white", attrs=['bold']))
